...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Extract code blocks that follow a trigger string from Markdown to YAML."
    )

    parser.add_argument(
        "-i", "--input", required=True, help="Path to input file OR directory of files."
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    def save_extracted_blocks(blocks, dest_path, split_files=False):
        ...
        for b in blocks:
            ...
            try:
                ...
                print(
                    f"Warning: Failed to parse block as YAML. Keeping as string. Error: {e}"
                )
                block_data.append(b)
    ...
    if os.path.isdir(args.input):
        ...
        for filename in os.listdir(args.input):
            if filename.endswith(".md"):
                ...
                replacements = {"{filename}": file_stem, "{firstword}": first_word}
                ...
                if blocks:
                    ...
                    print(
                        f"Processed: {filename} -> {dest_path} (Split: {should_split})"
                    )
                    count += 1
    ...
    # CASE 2: Input is a Single File
        ...
        if blocks:
            ...
            if os.path.isdir(args.output):
                print(
                    "Error: Input is a file, but output is a directory. Please specify a full output filename."
                )
                sys.exit(1)
